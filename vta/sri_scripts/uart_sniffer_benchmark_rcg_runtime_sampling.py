...
def run_and_collect(env, remote, network_id, network, target, log_file_path, samples=10, host_ip ='192.168.2.99',
                    file_prefix="axi_uart_sniffer_fifo", port="/dev/ttyUSB3", baud=921600, sca_log_dir="uart_sniffer_data/asp_dac/rcg/4x8x8_additional/",
        ...
        for i, network in enumerate(networks):
            ...
            if not os.path.exists(log_file_path):
                ...
                run_and_collect(env, remote, i, network, target, log_file_path, samples=num_samples, host_ip=host_ip,
                                file_prefix=file_prefix, port=port, baud=baud, sca_log_dir=sca_log_dir, ht_sampling_rate=ht_sampling_rate,
                                )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_file_dir", type=str,
                        default='profiling_results/uart_sniffer/asp_dac/rcg/2x16x16_100k/',
                        help="path to json file to store profiling data")
    parser.add_argument("--sca_log_dir", type=str,
                        default='uart_sniffer_data/asp_dac/rcg/2x16x16_100k/',
                        help="path to json file to store profiling data")
    parser.add_argument('--samples', type=int, default=5,
                        help='number of times to run each network')
    parser.add_argument('--data_file_prefix', type=str, default="axi_uart_sniffer_asp_dac_2x16x16_100k",
    ...
    parser.add_argument('--schedule_log_files_path', type=str, default='logs/tuning_logs/vta_2x16x16/*.log',
    ...
    connect_and_run(device='vta', networks=networks, log_file_dir=args.log_file_dir, host_ip=args.host_ip, num_samples=args.samples,
                    file_prefix=args.data_file_prefix, port=args.serial_port, baud=args.baud, sca_log_dir=args.sca_log_dir,

"""
Phase B Step 3 (Merged): Dense Ops on CPU Strategy
==================================================

This script executes dense matrix transformations on CPU (float32) while
keeping convolutions on VTA PL. Key differences from POC:

  1. Dense ops stay float32 end-to-end (no VTA quantization epilogues)
  2. No chunking of large dense ops (CPU can handle full tensors)
  3. Dense ops are hoisted to module top, before first conv
  4. graph_pack starts after max_pool2d as usual (ignores hoisted dense)
  5. Single cast to int8 at dense output boundary (where conv2d consumes)

This strategy avoids packing dense for VTA entirely, keeping VTA PL
focused on convolution execution. Expected improvement: faster inference
due to no dense-on-vta overhead.
"""
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "step3_results_dense_on_cpu")
...
def pick_subnets_from_sa(sa_file, arch_file, target_n=25, target_lambda=4.0, target_seed=0, k=1):
...
def _expr_dtype_no_checked_type(expr):
    """Get dtype string without requiring checked_type. Best-effort."""
    if isinstance(expr, relay.Constant):
        data = expr.data
        arr = data.asnumpy() if hasattr(data, "asnumpy") else np.asarray(data)
        return str(arr.dtype)
    if hasattr(expr, "type_annotation") and expr.type_annotation is not None:
        ta = expr.type_annotation
        if isinstance(ta, relay.TensorType):
            return str(ta.dtype)
    if isinstance(expr, relay.Var) and hasattr(expr, "type_annotation"):
        if isinstance(expr.type_annotation, relay.TensorType):
            return str(expr.type_annotation.dtype)
    return None


class _DenseOpEpilogueStripper(ExprMutator):
    """
    Strip VTA quantization epilogues from dense ops but keep epilogue structure.
    
    Key insight: Since dense runs on CPU (float32), we:
    1. Keep the dense with out_dtype="float32" (not int32 accumulation)
    2. Remove right_shift since there's no VTA-specific quantization
    3. Keep clip/cast/copy/stop_fusion chain but with float32->int8 boundary
    
    This avoids type mismatches while still producing int8 output where conv2d expects it.
    """
    ...
    def visit_call(self, call):
        ...
        # Check if this is a cast back to int32 after stop_fusion epilogue
        if not (isinstance(call.op, tvm.ir.Op) and call.op.name == "cast"):
            return new_call
        if call.attrs.dtype != "int32":
            return new_call

        arg = call.args[0]
        # Trace backwards through stop_fusion -> copy -> cast(int8) -> clip -> right_shift -> dense
        if not (isinstance(arg.op, tvm.ir.Op) and arg.op.name == "annotation.stop_fusion"):
            return new_call

        stop_fusion_arg = arg.args[0]
        if not (isinstance(stop_fusion_arg.op, tvm.ir.Op) and stop_fusion_arg.op.name == "copy"):
            return new_call

        copy_arg = stop_fusion_arg.args[0]
        if not (isinstance(copy_arg.op, tvm.ir.Op) and copy_arg.op.name == "cast"):
            return new_call
        if copy_arg.attrs.dtype != "int8":
            return new_call

        cast_i8_arg = copy_arg.args[0]
        if not (isinstance(cast_i8_arg.op, tvm.ir.Op) and cast_i8_arg.op.name == "clip"):
            return new_call

        clip_arg = cast_i8_arg.args[0]
        if not (isinstance(clip_arg.op, tvm.ir.Op) and clip_arg.op.name == "right_shift"):
            return new_call

        right_shift_arg = clip_arg.args[0]
        if not (isinstance(right_shift_arg.op, tvm.ir.Op) and right_shift_arg.op.name == "nn.dense"):
            return new_call

        # Found a dense epilogue! Rebuild with float32 dense -> int8 cast
        dense_call = right_shift_arg
        data, weight = dense_call.args
        units = dense_call.attrs.units if hasattr(dense_call.attrs, "units") else None

        # Create float32 dense (no quantization shift since it's on CPU)
        dense_f32 = relay.nn.dense(data, weight, units=units, out_dtype="float32")
        
        # Directly cast to int8 and wrap with stop_fusion
        cast_i8 = relay.cast(dense_f32, "int8")
        copied = relay.copy(cast_i8)
        stopped = relay.annotation.stop_fusion(copied)
        
        # Return as int32 for downstream (same as original epilogue pattern)
...
def _materialize_int8_pool_constants(mod_q, tvm_params_full, pool_var_names):
    ...
    # NEW: Strip dense epilogues (convert to float32, no int32 accumulation)
    body = _DenseOpEpilogueStripper().visit(body)
    
    # NEW: Add int8 casts where dense feeds conv2d
    body = _DenseOutputInt8CastAdder().visit(body)
...
# ============================================================
# Build / Compile / Run (adapted from POC)
# ============================================================
...
def step3b_compile_merged(
    ...
    sep("3B Compile Merged (Dense on CPU): %s" % subnet_id)
    ...
    if static_debug_mode:
        ...
        print("  Rewriting merged graph for int8 runtime-pool materialization + dense on CPU...")
    ...
    if enable_graph_pack:
        print("  Applying graph_pack (dense ops now on CPU, packing starts after max_pool2d)...")
        with tvm.transform.PassContext(opt_level=OPT_LEVEL, disabled_pass={"AlterOpLayout"}):
        ...
        print("  graph_pack: OK (dense ops excluded from packing)")
    else:
    ...
    return {
        ...
        "compile_strategy": "single-target(vta) + dense-on-cpu",
    }


def step3c_run_merged(subnet_id, arch, compile_artifacts, ofa_net, input_np, env, remote, ctx):
    sep("3C Run Merged (Dense on CPU): %s" % subnet_id)

    print("  Uploading library...")
    temp_dir = tvm_utils.tempdir()
    lib_path = temp_dir.relpath("graphlib_step3_merged_dense_on_cpu_%s.tar" % subnet_id)
    compile_artifacts["lib"].export_library(lib_path)
    remote.upload(lib_path)
    remote_lib = remote.load_module("graphlib_step3_merged_dense_on_cpu_%s.tar" % subnet_id)
...
# ============================================================
def parse_args():
    p = argparse.ArgumentParser(description="Step3 merged-module derivation+inference POC (dense on CPU)")
...
def main():
    args = parse_args()
    sep("Step3 Merged Mod Deriv POC (Dense on CPU)")
    ...
    print("  Strategy: Dense ops on CPU (float32), convolutions on VTA (int8)")
    print("\n[1] Loading OFA model...")
    t0 = time.time()
    ...
    summary_path = os.path.join(RESULTS_DIR, "step3_merged_summary_dense_on_cpu.json")

...
def test_gemm(test_name=None, batch=128, channel=128, block=128):
    def run_gemm_packed(env, remote, batch_size, channel, block):
        data_shape = (max(batch_size // env.BATCH, 1),
                      max(channel // env.BLOCK_IN, 1),
                      env.BATCH,
                      env.BLOCK_IN)
        weight_shape = (max(channel // env.BLOCK_OUT, 1),
                        max(channel // env.BLOCK_IN, 1),
                        env.BLOCK_OUT,
                        env.BLOCK_IN)
        res_shape = (max(batch_size // env.BATCH, 1),
                     max(channel // env.BLOCK_OUT, 1),
                     env.BATCH,
                     env.BLOCK_OUT)
        # To compute number of ops, use a x2 factor for FMA

        ko = te.reduce_axis((0, max(channel // env.BLOCK_IN, 1)), name='ko')
        ...
        def verify(s, check_correctness=True):
            ...
            ndata_shape = (max(batch_size//env.BATCH, 1)*env.BATCH, max(channel//env.BLOCK_IN, 1)*env.BLOCK_IN)
            nweight_shape = (max(channel//env.BLOCK_OUT, 1)*env.BLOCK_OUT,
                                max(channel//env.BLOCK_IN, 1)*env.BLOCK_IN)
            ndata_orig = np.zeros(ndata_shape).astype(data.dtype)
            nweight_orig = np.zeros(nweight_shape).astype(weight.dtype)
            ndata_orig[:data_orig.shape[0], :data_orig.shape[1]] = data_orig
            nweight_orig[:weight_orig.shape[0], :weight_orig.shape[1]] = weight_orig           
            data_packed = ndata_orig.reshape(
                max(batch_size // env.BATCH, 1), env.BATCH,
                max(channel // env.BLOCK_IN, 1), env.BLOCK_IN).transpose((0, 2, 1, 3))
            weight_packed = nweight_orig.reshape(
                max(channel // env.BLOCK_OUT, 1), env.BLOCK_OUT,
                max(channel // env.BLOCK_IN, 1), env.BLOCK_IN).transpose((0, 2, 1, 3))
            ...
            for b in range(max(batch_size // env.BATCH, 1)):
                for i in range(max(channel // env.BLOCK_OUT, 1)):
                    for j in range(max(channel // env.BLOCK_IN, 1)):
            ...
            res_unpack = res_arr.asnumpy().reshape(max(batch_size // env.BATCH, 1),
                                                   max(channel // env.BLOCK_OUT ,1),
                                                   env.BATCH,
                                                   env.BLOCK_OUT)
        ...
        def run_schedule(load_inp,
            ...
            if block:
                ...
                iblock = max(block // env.BLOCK_IN, 1)
                oblock = max(block // env.BLOCK_OUT,1)
                xbo, xco, xbi, xci = s[res].op.axis
                xb1, xco1, xb2, xco2 = s[res].tile(xbo, xco, 1, oblock)
                store_pt = xb2
        ...
        def gemm_normal(print_ir):
            ...
            with vta.build_config():
                run_test("GEMM_E2E", print_ir, True)
        ...
        def store_out_unittest(print_ir):
            ...
            def run_test(header, print_ir):
                ...
                print(header)
            with vta.build_config():
                run_test("ST_OUT_UNIT", print_ir)
            print("")

        if test_name is None or test_name == 'E2E':
            gemm_normal(False)
        if test_name is None or test_name == 'GEMM':
            gemm_unittest(False)
        if test_name is None or test_name == 'ALU':
            alu_unittest(False)
        if test_name is None or test_name == 'LD_INP':
            load_inp_unittest(False)
        if test_name is None or test_name == 'LD_WGT':
            load_wgt_unittest(False)
        if test_name is None or test_name == 'ST_OUT':
            store_out_unittest(False)

    def _run(env, remote):
        print(f"========GEMM {batch} {channel} {block}=========")
        run_gemm_packed(env, remote, batch, channel, block)

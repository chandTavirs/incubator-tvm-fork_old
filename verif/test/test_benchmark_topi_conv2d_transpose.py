...
def test_conv2d_transpose(device, workload=None):
    def _run(env, remote):
        if device == "vta":
            target = env.target
        ...
        with autotvm.tophub.context(target): # load pre-tuned schedule parameters
            for name, wl in dcgan_wklds:
                if workload is not None and name != workload:
                    continue
                print(name, wl)
                correct, _ = run_conv2d_transpose(env, remote, wl, target)
                assert(correct)

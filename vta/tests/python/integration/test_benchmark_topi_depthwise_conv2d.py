...
"""Testing topi depthwise conv2d operator for VTA"""
...
Workload = namedtuple("Conv2DWorkload",
                      ['batch', 'height', 'width', 'in_filter', 'channel_multiplier',
...
depthwise_wkls = [
    ...
    # ('test1',  Workload(env.BATCH, 5, 5, 32, 1, 3, 3, 1, 1, 1, 1)),
...
def run_depthwise_conv2d(env, remote, wl, target,
    ...
    assert wl.channel_multiplier == 1
    assert env.BATCH == 1
    ...
    if "arm_cpu" in target.keys:
        data_pack = False
        layout = "NCHW"
        depthwise_conv2d_fcompute = topi.arm_cpu.depthwise_conv2d_nchw
        depthwise_conv2d_fschedule = topi.arm_cpu.schedule_depthwise_conv2d_nchw
    elif "vta" in target.keys:
        data_pack = True
        layout = "NCHW%dn%dc" % (env.BATCH, env.BLOCK_OUT)
        depthwise_conv2d_fcompute = vta.top.depthwise_conv2d_packed
        depthwise_conv2d_fschedule = vta.top.schedule_depthwise_conv2d_packed

    # Derive shapes depending upon packing
    a_shape = (wl.batch, wl.in_filter, wl.height, wl.width)
    w_shape = (wl.in_filter, wl.channel_multiplier, wl.hkernel, wl.wkernel)
    b_shape = (wl.batch, wl.in_filter, 1, 1)
    if data_pack:
        data_shape = (wl.batch//env.BATCH, wl.in_filter//env.BLOCK_OUT,
                      wl.height, wl.width, env.BATCH, env.BLOCK_OUT)
        kernel_shape = (wl.in_filter//env.BLOCK_OUT, wl.channel_multiplier//env.BATCH,
                        wl.hkernel, wl.wkernel, env.BLOCK_OUT, env.BATCH)
        bias_shape = (wl.batch//env.BATCH, wl.in_filter//env.BLOCK_OUT,
    ...
    kernel = te.placeholder(kernel_shape, name="kernel", dtype=env.acc_dtype)
    ...
    with target:
        ...
        s = depthwise_conv2d_fschedule([res])
        if print_ir:
    ...
    def get_ref_data():
        ...
        r_np = topi.testing.depthwise_conv2d_python_nchw(
            a_np.astype(env.acc_dtype), w_np.astype(env.acc_dtype), (wl.hstride, wl.wstride), pad).astype(env.acc_dtype)
    ...
    if data_pack:
        data_np = data_np.reshape(
            ...
            wl.in_filter//env.BLOCK_OUT, env.BLOCK_OUT,
            wl.height, wl.width).transpose((0, 2, 4, 5, 1, 3))
        kernel_np = kernel_np.reshape(
            wl.in_filter//env.BLOCK_OUT, env.BLOCK_OUT,
            wl.channel_multiplier//env.BATCH, env.BATCH,
            wl.hkernel, wl.wkernel).transpose((0, 2, 4, 5, 1, 3))
        bias_np = bias_np.reshape(
            wl.batch//env.BATCH, wl.in_filter//env.BLOCK_OUT,
            1, 1, env.BATCH, env.BLOCK_OUT)
    ...
    if env.TARGET in ["sim", "tsim"]:
        ...
        if local_rpc:
            ...
            cost = f(data_arr, kernel_arr, bias_arr, res_arr)
            if env.TARGET == "sim":
    ...
    # LLVM Execution
    # cost = mod(data_arr, kernel_arr, res_arr)
    ...
    if check_correctness:
        ...
        if data_pack:
            res_orig = res_orig.transpose(
                (0, 4, 1, 5, 2, 3)).reshape(wl.batch, wl.in_filter, fout_height, fout_width)
            bias_np = bias_np.transpose(
                (0, 4, 1, 5, 2, 3)).reshape(wl.batch, wl.in_filter, 1, 1)
...
def test_conv2d(device):
    def _run(env, remote):
        ...
        with autotvm.tophub.context(target): # load pre-tuned schedule parameters
            for _, wl in depthwise_wkls:
                print(wl)
                run_depthwise_conv2d(env, remote, wl, target)

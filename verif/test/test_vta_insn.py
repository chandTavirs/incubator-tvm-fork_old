...
def test_padded_load(test_name=None):
    """Test padded load."""
    def _run(env, remote):
        ...
        if test_name is None or test_name == 'Y0':
          check_padded_load([2, 0, 0, 0], [0, 0, 0, 0], test_name="Y0")
        if test_name is None or test_name == 'Y1':
          check_padded_load([0, 2, 0, 0], [0, 0, 0, 0], test_name="Y1")
        if test_name is None or test_name == 'X0':
          check_padded_load([0, 0, 0, 0], [2, 0, 0, 0], test_name="X0")
        if test_name is None or test_name == 'X1':
...
def test_gemm(test_name=None):
    ...
    def _run(env, remote):
        ...
        if test_name is None or test_name == 'DEFAULT':
          test_schedule1()
        if test_name is None or test_name == 'SMT':
          test_smt()
    vta.testing.run(_run)


def test_alu(test_name=None):
    def _run(env, remote):
        def check_alu(tvm_op, np_op=None, is_mov=False, use_imm=False, test_name=None):
            """Test ALU"""
            m = 8
            n = 8
            ...
            if not is_mov:
                ...
                res_np = np.array(res_np, dtype=res.dtype)
        ...
        if test_name is None or test_name == 'SHL':
          check_alu(lambda x, y: x << y, np.left_shift, use_imm=True, test_name="SHL")
        if test_name is None or test_name == 'MAXI':
          check_alu(tvm.te.max, np.maximum, use_imm=True, test_name="MAX")
        if test_name is None or test_name == 'MAX':
          check_alu(tvm.te.max, np.maximum, test_name="MAX")
        if test_name is None or test_name == 'ADDI':
          check_alu(lambda x, y: x + y, use_imm=True, test_name="ADD")
        if test_name is None or test_name == 'ADD':
          check_alu(lambda x, y: x + y, test_name="ADD")
        if test_name is None or test_name == 'SHR':
          check_alu(lambda x, y: x >> y, np.right_shift, use_imm=True, test_name="SHR")
        if test_name is None or test_name == 'MOV':
          check_alu(lambda x, y: y, is_mov=True, test_name="MOV")
        if test_name is None or test_name == 'MOVI':
          check_alu(lambda x, y: y, use_imm=True, is_mov=True, test_name="MOVI")

...
def gemm_mat_trf(env, mock=False):
    """Matrix transform intrinsic for dense-transform subgraphs
    
    Computes: out = transform(inp, trf_matrix)
    where inp are packed input blocks and trf_matrix is a transformation matrix.
    ...
    # Transformation matrix (similar role to weight matrix)
    trf_mat = te.placeholder(
        (wgt_shape[0], wgt_shape[1]), dtype="int%d" % env.WGT_WIDTH, name=env.wgt_scope
    )
    # Input blocks (similar role to input)
    inp = te.placeholder(
        (inp_shape[0], inp_shape[1]), dtype="int%d" % env.INP_WIDTH, name=env.inp_scope
    )
    ...
    k = te.reduce_axis((0, wgt_shape[1]), name="k")
    out_dtype = "int%d" % env.ACC_WIDTH
    # Matrix transform: apply transformation to input blocks
    out = te.compute(
        (out_shape[0], out_shape[1]),
        lambda i, j: te.sum(inp[i, k].astype(out_dtype) * trf_mat[j, k].astype(out_dtype), axis=[k]),
        name="out",
    )
    ...
    def intrin_func(ins, outs):
        """Matrix transform intrinsic function"""
        dinp, dtrf_mat = ins
        dout = outs[0]

        def instr(index):
            """Generate matrix transform VTA instruction with GEMM_MAT_TRF mode flag"""
            irb = tvm.tir.ir_builder.create()
            dev = env.dev
            irb.scope_attr(dev.vta_axis, "coproc_scope", dev.get_task_qid(dev.QID_COMPUTE))
            irb.scope_attr(dev.vta_axis, "coproc_uop_scope", dev.vta_push_uop)
    ...
    return te.decl_tensor_intrin(
        out.op, intrin_func, name="GEMM_MAT_TRF", binds={inp: inp_layout, trf_mat: trf_mat_layout, out: out_layout}
    )

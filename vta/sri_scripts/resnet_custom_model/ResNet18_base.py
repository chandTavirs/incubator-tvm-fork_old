...
resnet18_base_orig_dense = nn.Sequential(b1, b2, b3, b4, b5,
                    nn.AdaptiveAvgPool2d((1, 1)),
                    nn.Flatten(), nn.Linear(512, 1000))

def get_resnet34_scratch():

    b1_34 = nn.Sequential(nn.Conv2d(3, 64, kernel_size=(7,7), stride=(2,2), padding=3),
                       nn.BatchNorm2d(64), nn.ReLU(inplace=True),
                       nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
    ...
    resnet34_scratch = nn.Sequential(b1_34, b2_34, b3_34, b4_34, b5_34,
                        nn.AdaptiveAvgPool2d((1, 1)),
                        nn.Flatten(), nn.Linear(512, 10))
    return resnet34_scratch
